...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	...
	// 是否延迟加载，对应bean 的属性 lazy - init
	@Nullable
	private Boolean lazyInit;

	// 自动注入模式，对应bean 属性 autowire
	private int autowireMode = AUTOWIRE_NO;

	// 依赖检查
	private int dependencyCheck = DEPENDENCY_CHECK_NONE;

	// 用来表示一个bean 的实例化依靠另一个bean 先实例化，对应bean 属性 depend-on
	@Nullable
	private String[] dependsOn;

	/**
	 * autowire-condidate 属性设置为false, 这样容器在查找自动装配对象时，将不会考虑该bean,
	 * 即它不会被考虑作为其他bean自动装配的候选者，但是该bean本身还是可以使用自动装配配注入其他bean 的。
	 * 对应 bean 的属性 autowire-condidate
	 * */
	private boolean autowireCandidate = true;

	// 自动装配时出现多个bean 候选者时，将作为首选者，对应bean 属性 primary
	private boolean primary = false;

	// 用于记录 Qualifier , 对应 元素qualifier
	...
	// 对应bean 属性 factory-bean
	@Nullable
	private String factoryBeanName;

	// 对应bean 属性 factory-method
	@Nullable
	...
	// 销毁方法， 对应bean 属性 destroy-method
	@Nullable
	private String destroyMethodName;

	// 是否执行init-method 方法，程序设置
